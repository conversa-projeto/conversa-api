...
function TWebSocketServer.Send(Context: TIdContext; sData: String): TWebSocketServer;
begin
  Result := Self;
  if Context.Connection.Connected then
    TWebSocketIOHandlerHelper(Context.Connection.IOHandler).WriteString(sData);
end;

function TWebSocketServer.SendAll(sData: String): TWebSocketServer;
var
  Clients: TList;
  I: Integer;
begin
...
function TWebSocketServer.Start: TWebSocketServer;
begin
  Result := Self;
  DefaultPort := FPort;
  Active := True;
...
function TWebSocketServer.Stop: TWebSocketServer;
begin
  Result := Self;
  Active := False;
end;

procedure TWebSocketServer.ReceiveText(AContext: TIdContext);
var
